...
- [尚硅谷\_MyCat课程讲义](#尚硅谷_mycat课程讲义)
	- [1. Mycat概述](#1-mycat概述)
		- [1.1 基本介绍](#11-基本介绍)
		- [1.2 Mycat作用](#12-mycat作用)
		- [1.3 原理](#13-原理)
	- [2. 使用前准备工作](#2-使用前准备工作)
	- [3. 安装启动](#3-安装启动)
		- [3.1 安装](#31-安装)
		- [3.2 配置与启动](#32-配置与启动)
		- [3.3 登录](#33-登录)
			- [1. 登录后台管理窗口](#1-登录后台管理窗口)
			- [2. 登录数据窗口](#2-登录数据窗口)
			- [3. 项目中登录](#3-项目中登录)
	- [4. 主从复制原理](#4-主从复制原理)
		- [4.1 主从复制原理](#41-主从复制原理)
		- [4.2 MySQL复制三步骤：](#42-mysql复制三步骤)
		- [4.3 复制的问题](#43-复制的问题)
		- [4.4 复制的基本原则](#44-复制的基本原则)
	- [5. 主从复制与读写分离的实现](#5-主从复制与读写分离的实现)
		- [5.1 搭建主从复制：一主一从](#51-搭建主从复制一主一从)
			- [1、搭建MySQL主从复制](#1搭建mysql主从复制)
			- [2、Mycat登录访问](#2mycat登录访问)
		- [5.2 实现一主一从的读写分离](#52-实现一主一从的读写分离)
			- [1、验证读写分离](#1验证读写分离)
			- [2、实现读写分离](#2实现读写分离)
			- [3、启动Mycat](#3启动mycat)
			- [4、验证读写分离](#4验证读写分离)
		- [5.3 搭建主从复制：双主双从](#53-搭建主从复制双主双从)
			- [0、储备](#0储备)
			- [1、 搭建MySQL主从复制（双主双从）](#1-搭建mysql主从复制双主双从)
		- [5.4 实现双主双从机的读写分离](#54-实现双主双从机的读写分离)
			- [1、修改配置文件](#1修改配置文件)
			- [2、启动Mycat](#2启动mycat)
			- [3、验证读写分离](#3验证读写分离)
			- [4、抗风险能力测试](#4抗风险能力测试)
	- [6. Mycat数据分片](#6-mycat数据分片)
		- [6.1 什么是数据分片？](#61-什么是数据分片)
		- [6.2 切分模式](#62-切分模式)
		- [6.3 Mycat分片原理](#63-mycat分片原理)
	- [6. 垂直拆分——分库](#6-垂直拆分分库)
		- [6.1 如何划分表](#61-如何划分表)
		- [6.2 实现分库](#62-实现分库)
			- [1、修改schema配置文件](#1修改schema配置文件)
			- [2、新增两个空白库](#2新增两个空白库)
			- [3、启动Mycat](#3启动mycat-1)
			- [4、访问Mycat进行分库](#4访问mycat进行分库)
	- [7. 水平拆分——分表](#7-水平拆分分表)
		- [7.1 实现分表](#71-实现分表)
			- [1、选择要拆分的表](#1选择要拆分的表)
			- [2、分表字段的考量](#2分表字段的考量)
			- [3、修改配置文件schema.xml](#3修改配置文件schemaxml)
			- [4、 修改配置文件rule.xml](#4-修改配置文件rulexml)
			- [5、在数据节点dn2上建orders表](#5在数据节点dn2上建orders表)
			- [6、重启Mycat让配置生效](#6重启mycat让配置生效)
			- [7、访问Mycat实现分片](#7访问mycat实现分片)
		- [7.2 Mycat 的分片 “join”](#72-mycat-的分片-join)
			- [1、ER表](#1er表)
			- [2、全局表](#2全局表)
		- [7.3 常用分片规则](#73-常用分片规则)
			- [方式1：取模](#方式1取模)
			- [方式2：分片枚举](#方式2分片枚举)
			- [方式3：范围约定](#方式3范围约定)
			- [方式4：按日期（天）分片](#方式4按日期天分片)
		- [7.4 全局ID序列](#74-全局id序列)
			- [方式1：本地文件（不推荐）](#方式1本地文件不推荐)
			- [方式2：数据库方式（推荐）](#方式2数据库方式推荐)
			- [方式3：时间戳方式（不推荐）](#方式3时间戳方式不推荐)
			- [方式4：自主生成全局序列](#方式4自主生成全局序列)
