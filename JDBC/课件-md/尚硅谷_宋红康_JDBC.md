...
- [JDBC核心技术](#jdbc核心技术)
	- [第1章：JDBC概述](#第1章jdbc概述)
		- [1.1 数据的持久化](#11-数据的持久化)
		- [1.2 Java中的数据存储技术](#12-java中的数据存储技术)
		- [1.3 JDBC介绍](#13-jdbc介绍)
		- [1.4 JDBC体系结构](#14-jdbc体系结构)
		- [1.5 JDBC程序编写步骤](#15-jdbc程序编写步骤)
	- [第2章：获取数据库连接](#第2章获取数据库连接)
		- [2.1 要素一：Driver接口实现类](#21-要素一driver接口实现类)
			- [2.1.1 Driver接口介绍](#211-driver接口介绍)
			- [2.1.2 加载与注册JDBC驱动](#212-加载与注册jdbc驱动)
		- [2.2 要素二：URL](#22-要素二url)
		- [2.3 要素三：用户名和密码](#23-要素三用户名和密码)
		- [2.4 数据库连接方式举例](#24-数据库连接方式举例)
			- [2.4.1 连接方式一](#241-连接方式一)
			- [2.4.2 连接方式二](#242-连接方式二)
			- [2.4.3 连接方式三](#243-连接方式三)
			- [2.4.4 连接方式四](#244-连接方式四)
			- [2.4.5 连接方式五(最终版)](#245-连接方式五最终版)
	- [第3章：使用PreparedStatement实现CRUD操作](#第3章使用preparedstatement实现crud操作)
		- [3.1 操作和访问数据库](#31-操作和访问数据库)
		- [3.2 使用Statement操作数据表的弊端](#32-使用statement操作数据表的弊端)
		- [3.3 PreparedStatement的使用](#33-preparedstatement的使用)
			- [3.3.1 PreparedStatement介绍](#331-preparedstatement介绍)
			- [3.3.2 PreparedStatement vs Statement](#332-preparedstatement-vs-statement)
			- [3.3.3 Java与SQL对应数据类型转换表](#333-java与sql对应数据类型转换表)
			- [3.3.4 使用PreparedStatement实现增、删、改操作](#334-使用preparedstatement实现增删改操作)
			- [3.3.5 使用PreparedStatement实现查询操作](#335-使用preparedstatement实现查询操作)
		- [3.4 ResultSet与ResultSetMetaData](#34-resultset与resultsetmetadata)
			- [3.4.1 ResultSet](#341-resultset)
			- [3.4.2 ResultSetMetaData](#342-resultsetmetadata)
		- [3.5 资源的释放](#35-资源的释放)
		- [3.6 JDBC API小结](#36-jdbc-api小结)
	- [章节练习](#章节练习)
	- [第4章 操作BLOB类型字段](#第4章-操作blob类型字段)
		- [4.1 MySQL BLOB类型](#41-mysql-blob类型)
		- [4.2 向数据表中插入大数据类型](#42-向数据表中插入大数据类型)
		- [4.3 修改数据表中的Blob类型字段](#43-修改数据表中的blob类型字段)
		- [4.4 从数据表中读取大数据类型](#44-从数据表中读取大数据类型)
	- [第5章 批量插入](#第5章-批量插入)
		- [5.1 批量执行SQL语句](#51-批量执行sql语句)
		- [5.2 高效的批量插入](#52-高效的批量插入)
			- [5.2.1 实现层次一：使用Statement](#521-实现层次一使用statement)
			- [5.2.2 实现层次二：使用PreparedStatement](#522-实现层次二使用preparedstatement)
			- [5.2.3 实现层次三](#523-实现层次三)
			- [5.2.4 实现层次四](#524-实现层次四)
	- [第6章： 数据库事务](#第6章-数据库事务)
		- [6.1 数据库事务介绍](#61-数据库事务介绍)
		- [6.2 JDBC事务处理](#62-jdbc事务处理)
		- [6.3 事务的ACID属性](#63-事务的acid属性)
			- [6.3.1 数据库的并发问题](#631-数据库的并发问题)
			- [6.3.2 四种隔离级别](#632-四种隔离级别)
			- [6.3.3 在MySql中设置隔离级别](#633-在mysql中设置隔离级别)
	- [第7章：DAO及相关实现类](#第7章dao及相关实现类)
		- [【BaseDAO.java】](#basedaojava)
		- [【BookDAO.java】](#bookdaojava)
		- [【UserDAO.java】](#userdaojava)
		- [【BookDaoImpl.java】](#bookdaoimpljava)
		- [【UserDaoImpl.java】](#userdaoimpljava)
		- [【Book.java】](#bookjava)
		- [【Page.java】](#pagejava)
		- [【User.java】](#userjava)
	- [第8章：数据库连接池](#第8章数据库连接池)
		- [8.1 JDBC数据库连接池的必要性](#81-jdbc数据库连接池的必要性)
		- [8.2 数据库连接池技术](#82-数据库连接池技术)
		- [8.3 多种开源的数据库连接池](#83-多种开源的数据库连接池)
			- [8.3.1 C3P0数据库连接池](#831-c3p0数据库连接池)
			- [8.3.2 DBCP数据库连接池](#832-dbcp数据库连接池)
			- [8.3.3 Druid（德鲁伊）数据库连接池](#833-druid德鲁伊数据库连接池)
	- [第9章：Apache-DBUtils实现CRUD操作](#第9章apache-dbutils实现crud操作)
		- [9.1 Apache-DBUtils简介](#91-apache-dbutils简介)
		- [9.2 主要API的使用](#92-主要api的使用)
			- [9.2.1 DbUtils](#921-dbutils)
			- [9.2.2 QueryRunner类](#922-queryrunner类)
			- [9.2.3 ResultSetHandler接口及实现类](#923-resultsethandler接口及实现类)
	- [JDBC总结](#jdbc总结)
